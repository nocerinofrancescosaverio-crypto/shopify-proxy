...
app.get("/app",(req,res)=>{
  ...
  const html = [
    ...
    '</div>',
    '<div id="tab-csv">',
    '<div class="dz" id="dropzone" onclick="document.getElementById(\'fInput\').click()"',
    ...
    '</div>',
    '<p style="font-size:10px;color:#333;margin-top:10px">Colonne: title, vendor, product_type, tags, price, sku, inventory_quantity, status, description</p>',
    '</div>',
    ...
    '<div><label class="lbl">Quantita</label><input id="mQty" placeholder="10"/></div>',
    ...
    'function addManual(){var t=document.getElementById("mTitle").value;if(!t)return addLog("Titolo obbligatorio","error");prods.push({title:t,vendor:document.getElementById("mVendor").value,ptype:document.getElementById("mType").value,tags:document.getElementById("mTags").value,price:document.getElementById("mPrice").value,sku:document.getElementById("mSku").value,qty:document.getElementById("mQty").value,status:document.getElementById("mStatus").value,desc:document.getElementById("mDesc").value});["mTitle","mVendor","mType","mTags","mPrice","mSku","mQty","mDesc"].forEach(function(id){document.getElementById(id).value=""});addLog("Aggiunto: "+t,"success");document.querySelectorAll(".tab")[2].click();renderList();updateStats()}',
    'function handleCSV(f){if(!f)return;var reader=new FileReader();reader.onload=function(e){var text=e.target.result;var lines=text.replace(/\\r/g,"").split("\\n");var h=lines[0].split(",").map(function(x){return x.trim().replace(/"/g,"").toLowerCase()});var ps=lines.slice(1).filter(function(l){return l.trim()}).map(function(l){var v=l.split(",").map(function(x){return x.trim().replace(/"/g,"")});var o={};h.forEach(function(k,i){o[k]=v[i]||""});return{title:o.title||"",vendor:o.vendor||"",ptype:o.product_type||"",tags:o.tags||"",price:o.price||"0",sku:o.sku||"",qty:o.inventory_quantity||"0",status:(o.status||"ACTIVE").toUpperCase(),desc:o.description||o.body_html||""}}).filter(function(p){return p.title});if(!ps.length)return addLog("Nessun prodotto trovato","error");prods=ps;sts={};addLog(ps.length+" prodotti caricati","success");document.querySelectorAll(".tab")[2].click();renderList();updateStats()};reader.readAsText(f)}',
    'async function upload(){var store=document.getElementById("iStore").value;var token=document.getElementById("iToken").value;if(!store||!token)return addLog("Inserisci store e token!","error");if(!prods.length)return addLog("Nessun prodotto","error");for(var i=0;i<prods.length;i++){if(sts[i]==="success")continue;sts[i]="loading";renderList();var p=prods[i];try{var tags=p.tags?p.tags.split(",").map(function(t){return t.trim()}):[];var body={query:"mutation productCreate($input:ProductInput!){productCreate(input:$input){product{id title}userErrors{field message}}}",variables:{input:{title:p.title,descriptionHtml:p.desc,vendor:p.vendor,productType:p.ptype,tags:tags,status:p.status,variants:[{price:p.price||"0",sku:p.sku,inventoryQuantities:[{availableQuantity:parseInt(p.qty)||0,locationId:"gid://shopify/Location/1"}]}]}}};var res=await fetch("/shopify",{method:"POST",headers:{"Content-Type":"application/json","X-Shopify-Store":store,"X-Shopify-Token":token},body:JSON.stringify(body)});var d=await res.json();var ue=d.data&&d.data.productCreate?d.data.productCreate.userErrors:[];if(ue.length)throw new Error(ue.map(function(e){return e.message}).join(", "));sts[i]="success";addLog("Creato: "+p.title,"success")}catch(e){sts[i]="error";addLog("Errore "+p.title+": "+e.message,"error")}renderList();updateStats();await new Promise(function(r){setTimeout(r,300)})}addLog("Upload completato!","success")}',
    '<\/script>',
    '</body></html>'
  ].join("\n");
